...
class TopoGCN(nn.Module):
    def __init__(self, in_channels=1, n_cls=10, graph_classification=True, beta=0.1):
        super(TopoGCN, self).__init__()
        self.graph_classification = graph_classification
        self.conv1 = GCNConv(in_channels, 32)
        self.togl = TOGLWithSelfAttention(32, 8, 32, 32, "mean", beta)
...
def main():
    ...
    parser.add_argument('--beta', type=float, default=0.1, help='Threshold')
    ...
    model = TopoGCN(in_channels, num_classes, graph_classification, args.beta).to(device)
