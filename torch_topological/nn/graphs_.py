...
class TOGLWithSelfAttention(nn.Module):
    def __init__(self, n_features, n_filtrations, hidden_dim, out_dim, aggregation_fn, edge_threshold=100):
        ...
        self.top_k_edges = nn.Parameter(torch.tensor(50.0))  # 初始化为50
    ...
    def forward(self, x, data):
        ...
        # 使用softmax将top_k_edges转化为概率
        top_k_prob = torch.softmax(self.top_k_edges, dim=0)

        # 根据概率选择边
        _, topk_indices = torch.topk(attention_weights, int(top_k_prob.item()))

        # Create subgraph with top-k key edges
        edge_index = edge_index[:, topk_indices]
        subset = torch.unique(edge_index)
        sub_x = x[subset]
        dense_adj = to_dense_adj(edge_index, max_num_nodes=subset.size(0))
